...
Both cemeteries are new to the source (verified: no existing row for either
name+plaats combination) and both marked "(geruimd)" in the placemark name,
same convention as the first batch. Reuses the same parsing/splitting logic
as scripts/add_tijdelijk_zuidholland_kmz.py.
...
KMZ_PATH = REPO_ROOT / "data" / "Tijdelijk Zuid-Holland 2.kmz"
...
def main() -> None:
    ...
    assert len(cemeteries) == 2, f"verwacht 2 begraafplaatsen in de kmz, gevonden {len(cemeteries)}"
